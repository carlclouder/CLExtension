...
#define UseBnTestOpen  0
#define UseXyTestOpen  0
...
struct BPN{
	...
	struct exBPN
	{
		...
		Uint& mi;
		exBPN() = delete;
		exBPN(const exBPN&) = delete;
		explicit exBPN(BPN* pbn) :
			index(pbn->index),
			Eu(pbn->Eu), Ea2(pbn->Ea2), //SqrtEa2(pbn->SqrtEa2),
			ri(pbn->ri), bt(pbn->bt),
			mi(pbn->mi),
			u(pbn->Eu), a2(pbn->Ea2),
			Dri(pbn->Dri), Dbt(pbn->Dbt)
		{};
		void resetSize(Uint _startIndex, Uint _mi, Uint _units, Uint _span) {			
			startIndex = _startIndex;
			units = max(_units, 1);
			span = max(_span, 1) ;
			auto _si = Uint(max(_mi, 1)) * units * span;
			xi.clear(), xi.resize(_si, 0);
			xi_h.clear(), xi_h.resize(_si, 0);
			dy.clear(), dy.resize(_si, 0);
			//u = 0; a2 = 1;
			ci = 0; base = 0;
			nSiTimes = 0;
			//si = 0;
		...
		Float getYi_h(Uint index = 0, Uint l = 0) {
			auto pos = base + (index - startIndex) * span + l;
			//return xi2[pos] = ri * (xi[pos] - u) * pram1 + bt;
			return ri * (xi_h[pos] = (xi[pos] - u) * dsqrta2) + bt;
		}

		void forwardUpdate() {
			if (nSiTimes < BN_TOP_MI)
				return;
			//if (si < (mi * units * span))
			//	si = (ci + 1) * units * span;
			Float sun_si = 0;	
			...
			auto SqrtEa2 = sqrt(Ea2 * si / (si - 1) + VtEpslon);
			//auto SqrtEa2 = sqrt(Ea2 + VtEpslon);
			Dri = ri / SqrtEa2;
			Dbt = bt - ri * Eu / SqrtEa2;			
		}
		...
		void createBackwardParam() {
			...
			Float sum_dyXiu = 0;
			for (Uint i = 0; i < si; i++) {
				sum_dyXiu += dy[i] * (xi[i] - u);
			}
			dl_da2 = ri * sum_dyXiu * (-0.5) * pow(a2 + VtEpslon, (-1.5f));

			/*Float sum_dyXiu2 = 0;
			for (Uint i = 0; i < si; i++) {
				sum_dyXiu2 += ri * dy[i] * (xi[i] - u) * (-0.5) * pow(a2 + VtEpslon, (-1.5f));
			}*/
			...
			dl_du = sun_dy * ri * (-1.0f) * dsqrta2 + dl_da2 * (-2.0f) * sun_xiu / si;
		}
		Float gradSendOut(Uint index, Uint l = 0) {
			auto pos = base + (index - startIndex) * span + l;
			return ri * dy[pos] * dsqrta2 + dl_da2 * 2.0 * (xi[pos] - u) / si + dl_du / si;
		}
		void backwardUpdate() {
	...
	BPN(Uint _mi = 1, Float _ri = 1, Float _bt = 0, Float _Eu = 0,Float _Ea2 = 1) {
		ex = nullptr;
		//hwS = hwE = 0;
		set(_mi , _ri ,_bt,_Eu ,_Ea2);		
	}
	~BPN() { releaseEx(); }
	/*BPN& setRange(Uint _hws, Uint _hwe) {
		hwS = _hws; hwE = _hwe;
		return *this;
	}*/
	BPN& set(/*Uint _hws, Uint _hwe, */Uint _mi, Float _ri = 1, Float _bt = 0, Float _Eu = 0, Float _Ea2 = 1) {
		setBatch(_mi);
		//setRange(_hws,_hwe);
		Eu = _Eu;
		Ea2 = _Ea2;
	...
	BPN& resetSize(Uint _startIndex,/* Uint _hws, Uint _hwe,*/ Uint _mi, Uint units, Uint _span) {
		setBatch(_mi);
		//setRange(_hws, _hwe);
		//Eu = 0;
		//SqrtEa2 = Ea2 = 1;
		...
		//bt = 0;
		if (ex)
			ex->resetSize(_startIndex, mi, units, _span);
		return *this;
	}
	Bool isNeedToResetSize(Uint _units, Uint _span) const {
	...
	inline Float getYi_h(Uint index = 0, Uint l = 0) {
		return ex->getYi_h(index, l);
	}
...
#define _WcFuncParam_def const Uint index,const neuron& nn,const Float* preLayYiData0,const Uint span,const Uint l,NeuronExData* exData, const Uint exdataSpan, const Uint exdatal
	...
	Float Er(const Float* target, Uint targetSi) const;
	Float _Er(const Float* pyiData, const Float* target, Uint targetSi) const;

	Bool writeBpnnToFile(PCStr lpFileFullPathName , Bool binMode , Bool encrypteMod);
	Bool readBpnnFormFile(PCStr lpFileFullPathName, Bool binMode );
	...
	//损失函数
	Float loss(const Float y, const Float t) const;
	Bool getOutput(VLF& out_yi) const;
	Bool _getOutput(const Float* pyiData, VLF& out_yi) const;
	...
	PCStr getName() const;
	void* showWbDataDistribution(Int x, Int y, Uint cx, Uint cy, Uint sectionCounts) const;
	void exportWbDataDistribution(VLF& retRange, VLF& retFreq, Uint sectionCounts, Float* vmin, Float* vmax) const;
};
...
public:
	...
	//Bool bAutoFit;
	Bool bSetParam;
	Bpnn::PCBAutoFitLsAndMc pFAutoFit;
	PVoid pAutoFitOperateIns;
	...
	Float m_adamAlpha;//adam的步长
	//const Float m_adamDelta1 = 0.9;
	//const Float m_adamDelta2 = 0.999;
		
	CLAtomic<EBP_TPC> m_trainProcControl;//流程控制
	DWORD m_trainProcControl_postTrd;
	...
	template<int _i = 1, class _ty = Float>
	struct DataArray { 
		_ty data[_i];
		void reset() { ZeroMemory(this, sizeof(DataArray)); }
		DataArray() { reset(); }
		operator _ty() const { return data[0]; }
		operator _ty&() { return data[0]; }
		_ty operator ()(size_t i = 0) const { return data[i]; }
		_ty& operator ()(size_t i = 0){ return data[i]; }
		_ty operator[](size_t i) const { return data[i]; }
		_ty& operator[](size_t i) { return data[i]; }
		template<class _ty2>DataArray& operator=(_ty2 v) { return data[0] = v,*this; }
	};

	VLF vm_wji_dt_Data;
	/*VLF vm_wji_dt_old_Data;
	VLF vm_bi_dt_old_Data;*/
	vector<DataArray<2>> vm_wji_dt_old_Data;
	vector<DataArray<2>> vm_bi_dt_old_Data;
...
#define _gradient_param     index,lay,pos,nn,is,ie
	...
	Bool __runByHardwareSpeedup(VLF* pOutEa = nullptr, VLF* pOutLs = nullptr, VLF* pOutMc = nullptr, Bpnn::PCBMonitor _pCbFun = nullptr, PVoid _pIns = nullptr);
	Bool __runByHardwareSpeedup2(VLF* pOutEa = nullptr, VLF* pOutLs = nullptr, VLF* pOutMc = nullptr, Bpnn::PCBMonitor _pCbFun = nullptr, PVoid _pIns = nullptr);
...
public:			
	...
	private:
	//默认构造函数
	CLBpExtend(CLBpKernel* pkernel,const Bpnn& bp);
	...
	CLBpExtend& setAutoFitLsAndMc(/*Bool bOpen = false, */Bpnn::PCBAutoFitLsAndMc pFAutoFit = nullptr,PVoid pIns = nullptr);
	...
	CLBpExtend& buildNet(Bpnn::PCBMonitor _pCbFun = nullptr, PVoid _pIns = nullptr);

	Bool train(VLF* pOutEa = nullptr, VLF * pOutLs = nullptr, VLF * pOutMc = nullptr, Bpnn::PCBMonitor _pCbFun = nullptr, PVoid _pIns = nullptr);
