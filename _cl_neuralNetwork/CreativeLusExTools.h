		...
		for (Int i = 0; i < number_of_images; ++i) {
			...
			for (Int r = 0; r < n_rows; ++r) {
				for (Int c = 0; c < n_cols; ++c) {
					...
					pair.iv()[(width_src_image + 2 * x_padding) * (r + y_padding) + c + x_padding] = Float((temp / 255.0) * (scale_max - scale_min) + scale_min);
				}
			}
		}
		file.close();
			...
			for (Int d = 0; d < deep_src_image; ++d) {
				...
				for (Int r = 0; r < height_src_image; ++r) {
					...
					for (Int c = 0; c < width_src_image; ++c) {
						temp = 0;
						file.read((char*)&temp, sizeof(temp));
						pair.iv()[depSi + rowSi + c + x_padding] = Float((temp / 255.0) * (scale_max - scale_min) + scale_min);
					}
				}
			}
		}
