...
void CLString::initialize(LONG_PTR nDefaultCharNumber)
{
	...
	if (nDefaultCharNumber < 1) {
		...
		pHead = new TCHAR[m_unitNumber = nDefaultCharNumber + 1];
		pHead[0] = 0;
	}
	m_strlen = 0;
	m_changeFlag = FALSE;
...
CLString::PCLStringExData CLString::makeNewTempStringBuf(size_t stringSizeNoEndZero, size_t charSize)
{
	auto pEx = makeDEx();
	auto newsi = (LONG_PTR)((stringSizeNoEndZero + 1) * charSize);
	if (pEx->m_pointerBufSizeInByte_ < newsi)
		delete[] pEx->m_pointer_.pMultiByte, pEx->m_pointer_.pMultiByte = 0;
	if (!pEx->m_pointer_.pMultiByte)
		pEx->m_pointer_.pMultiByte = new char[pEx->m_pointerBufSizeInByte_ = newsi];
	if (m_encoding == EnCode_ASCII)
		pEx->m_PtExSaveType_ = EnCode_ASCII;
	else
		pEx->m_PtExSaveType_ = isBigEndian() ? EnCode_UBE : EnCode_ULE;
	return pEx;
}
...
CLString& CLString::swap(CLString& other)
{
	...
	std::swap(m_changeFlag, other.m_changeFlag);
	std::swap(m_encoding, other.m_encoding);
	std::swap(pDataEx, other.pDataEx);
	return *this;
}
...
LPCTSTR CLString::findString(LPCTSTR lpszSub, LONG_PTR nStart)
{
	auto i = find(lpszSub, nStart);
	if (i == -1)return _T("\0");
	auto pEx = makeNewTempStringBuf(size() - i);
	_tcscpy_s((LPTSTR)pEx->m_pointer_.pMultiByte, size() - i + 1, pHead + i);
	return (LPCTSTR)pEx->m_pointer_.pMultiByte;
}
CLString CLString::findStringObj(LPCTSTR lpszSub, LONG_PTR nStart)
{
	return findString(lpszSub, nStart);
}
CLString& CLString::findStringSave(LPCTSTR lpszSub, LONG_PTR nStart)
{
	findString(lpszSub, nStart);
	saveExStore();
	return *this;
}
LPCTSTR CLString::rfindString(LPCTSTR lpszSub, LONG_PTR nStart)
{
	auto i = rfind(lpszSub, nStart);
	if (i == -1)return _T("\0");
	auto pEx = makeNewTempStringBuf(size() - i);
	_tcscpy_s((LPTSTR)pEx->m_pointer_.pMultiByte, size() - i + 1, pHead + i);
	return (LPCTSTR)pEx->m_pointer_.pMultiByte;
}
CLString CLString::rfindStringObj(LPCTSTR lpszSub, LONG_PTR nStart)
{
	return rfindString(lpszSub, nStart);
}
CLString& CLString::rfindStringSave(LPCTSTR lpszSub, LONG_PTR nStart)
{
	rfindString(lpszSub, nStart);
	saveExStore();
	return *this;
}
LPCTSTR CLString::findMidString(LPCTSTR lpszSubLeft, LPCTSTR lpszSubRight, FMS_TYPE cutType, FMS_TYPE saveType,
	LONG_PTR nStart, LONG_PTR nRevStart)
{
	...
	case FMS_TYPE::FMS_MIN:
		...
		break;
	default:
		throw std::invalid_argument("saveType is error!");
		break;
	}
	size_t nchar = (st > -1 && et > -1 && st <= et) ? et - st : (st = et = 0);
	auto pex = makeNewTempStringBuf(nchar, sizeof(TCHAR));
	_tcsncpy_s((LPTSTR)pex->m_pointer_.pMultiByte, nchar + 1, pHead + st, nchar);
	return (LPCTSTR)pex->m_pointer_.pMultiByte;
}
CLString CLString::findMidStringObj(LPCTSTR lpszSubLeft, LPCTSTR lpszSubRight, FMS_TYPE cutType, FMS_TYPE saveType, LONG_PTR nStart, LONG_PTR nRevStart)
{
	return findMidString(lpszSubLeft, lpszSubRight, cutType, saveType, nStart, nRevStart);
}
CLString& CLString::findMidStringSave(LPCTSTR lpszSubLeft, LPCTSTR lpszSubRight, FMS_TYPE cutType, FMS_TYPE saveType, LONG_PTR nStart, LONG_PTR nRevStart)
{
	findMidString(lpszSubLeft, lpszSubRight, cutType, saveType, nStart, nRevStart);
	return saveExStore();
}
